...
function initMap(id, routeName) {
	...
	var str = 'http://cyclingworld-service.cfapps.io/rest/getRoute/' + id + '/'
	...
	xhr.onreadystatechange = function() {
		if (xhr.readyState == 4 && xhr.status == 200) {
			...
			if (responseRoute.coords.length > 0) {
				markers = [];
				vmarkers = [];
				var mapOptions = {
					zoom : 15,
				...
				map = new g.Map(document.getElementById("mapcontainer"),
						mapOptions);

				coords = [];
				for ( var i in responseRoute.coords) {
					coords.push(new google.maps.LatLng(
							responseRoute.coords[i].lat,
